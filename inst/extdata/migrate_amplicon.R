...
#
# Run this on any den created before gopheR 0.6.0.
...
if ("primer_set" %in% existing_tables) {
  cols <- existing_columns("primer_set")
  if (is.integer(DBI::dbGetQuery(con, "SELECT typeof(primer_set_id) FROM pragma_table_info('primer_set') WHERE name='primer_set_id'")[[1]])) {
    ...
    DBI::dbExecute(con, "CREATE INDEX IF NOT EXISTS idx_primer_set_marker_region ON primer_set(marker, region)")
...
if (!("primer_set_id" %in% existing_columns("asv"))) {
  ...
  DBI::dbExecute(con, "ALTER TABLE asv ADD COLUMN primer_set_id TEXT REFERENCES primer_set(primer_set_id)")
...
# ---------------------------------------------------------------------------
asv_cluster_pk <- DBI::dbGetQuery(con, "SELECT sql FROM sqlite_master WHERE type='table' AND name='asv_cluster'")$sql
if (grepl("CHECK", asv_cluster_pk) || !grepl("is_representative", asv_cluster_pk)) {
  ...
  DBI::dbExecute(con, "CREATE INDEX idx_asv_cluster_cluster ON asv_cluster(cluster_type, cluster_id)")
  if (nrow(old_rows) > 0 && all(c("workflow_id") %in% names(old_rows))) {
...
insert_if_missing <- function(con, table, where_col, where_val, insert_sql) {
  n <- DBI::dbGetQuery(con, sprintf("SELECT COUNT(*) FROM %s WHERE %s = ?", table, where_col), params = list(where_val))[[1]]
  if (n == 0) DBI::dbExecute(con, insert_sql)
}

insert_if_missing(con, "object_type", "object_type", "asv_batch",
  "INSERT INTO object_type VALUES ('asv_batch', 'A batch of ASV generation from a set of samples (e.g. one DADA2 run)')")

# Remove old technology-based readset subtypes (replaced by library-type subtypes)
...
edge_rows <- list(c("readset", "asv_batch", "derived_from", "asv_batch was generated from this readset"))
for (e in edge_rows) {
  n <- DBI::dbGetQuery(con,
  ...
  if (n == 0)
    DBI::dbExecute(con,
      "INSERT INTO edge_spec VALUES (?, ?, ?, ?)", params = as.list(e))
}
...
result_rows <- list(
  c("asv_batch", "total_asvs",       "integer", "Total ASVs before prevalence filtering",      NA),
  c("asv_batch", "filtered_asvs",    "integer", "ASVs retained after prevalence filtering",     NA),
  c("asv_batch", "filter_threshold", "text",    "Prevalence filter applied, e.g. 2x2 or 5x5",  NA),
  c("asv_batch", "median_depth",     "numeric", "Median per-sample sequencing depth",           NA),
  c("readset",   "technology",       "text",    "Sequencing technology: illumina, pacbio, nanopore", NA),
  c("readset",   "layout",           "text",    "Library layout: paired_end, single_end, long_read", NA)
)
...
# ---------------------------------------------------------------------------
# 6. Update existing readset subtypes: paired_end -> shotgun
#    (ARW metagenomics readsets were ingested as paired_end)
# ---------------------------------------------------------------------------
n_paired <- DBI::dbGetQuery(con,
  "SELECT COUNT(*) FROM object WHERE object_type='readset' AND object_subtype='paired_end'")[[1]]

if (n_paired > 0) {
  cli::cli_alert_info("Found {n_paired} readset:paired_end object(s) — updating to readset:shotgun")
  cli::cli_alert_info("Review before confirming: these are assumed to be shotgun metagenomics readsets.")
  cli::cli_alert_info("If any are amplicon readsets incorrectly labeled paired_end, update them manually.")
  DBI::dbExecute(con,
    "UPDATE object SET object_subtype = 'shotgun' WHERE object_type = 'readset' AND object_subtype = 'paired_end'")
...
# ---------------------------------------------------------------------------
db_path <- gopheR::get_db_path()
den_root <- dirname(db_path)
clustering_dir <- file.path(den_root, "archive", "clustering")
if (!dir.exists(clustering_dir)) {
  dir.create(clustering_dir, recursive = TRUE)
  cli::cli_alert_success("Created {.path {clustering_dir}}")
} else {
...
cli::cli_alert_success("Migration complete — den is ready for gopheR 0.6.x amplicon features")
